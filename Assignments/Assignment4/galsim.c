...
const float epsilon = 1e-3, circleRadius=0.01, circleColor=0, L=1, W=1;
...
void calculateParticleMotionOptimizedSoA1(particles_soa_t * restrict particles, int N, float delta_t) {
    ...
    const double Gdelta = (-100.0 * delta_t) / N;
    const int BLOCK_SIZE = 32;  // Adjust based on your cache size
...
int main(int argc, const char* argv[]) {
    ...
    if (graphics == 1){
        ...
        for (int k = 0; k < nsteps; k++){
            ...
            ClearScreen();
            for (int i = 0; i < N; i++){
                DrawCircle(particles_a.pos_dx[i], particles_a.pos_dy[i], L, W, circleRadius, circleColor);
            }
            Refresh();
